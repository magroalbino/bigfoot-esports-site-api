        ...
        while ((match = itemRegex.exec(xmlText)) !== null && news.length < 10) {
            ...
            if (titleMatch && linkMatch) {
                ...
                let content = contentMatch
                    ? contentMatch[1].replace(/<!\[CDATA\[(.*?)\]\]>/g, '$1')
                    : descriptionMatch
                    ? descriptionMatch[1].replace(/<!\[CDATA\[(.*?)\]\]>/g, '$1')
                    : '';
...
export function getStaticNews() {
    ...
    const today = new Date(); // Usar data atual do sistema
    const news = [
        {
...
export default async function handler(req, res) {
    ...
    try {
        console.log('API /api/news: Buscando notícias estáticas...');
        const staticNews = getStaticNews();
        console.log(`API /api/news: Retornando ${staticNews.length} notícias`);
