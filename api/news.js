...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Traduzir notícias para português
    const translatedNews = await Promise.all(
      news.slice(0, 15).map(async (item) => {
        try {
          const translatedTitle = await translateText(item.title);
          const translatedContent = item.content ? await translateText(item.content) : null;
    ...
    translatedNews.sort((a, b) => new Date(b.date) - new Date(a.date));
